...
EPSSApp.service('UtilService', ['$q', '$http', '$rootScope',
	function ($q, $http, $rootScope) {
		...
		this.getResponseError = function getResponseError(response, url, verbo) {
			...
			if (data !== null && data !== undefined)
				if (data.hasOwnProperty('mensajes'))
					responseError = data; // Datos adicionales informados y bien formateados desde el BackEnd.
				else
					responseError.mensajes.push({ codigo: 'Datos adicionales ', mensaje: data }); //Error informado y sin formato desde BackEnd.
			else
				responseError.mensajes.push({ codigo: 'Datos adicionales ', mensaje: '[sin datos adicionales]' }); //Error sin datos adicionales informados.
			//Agregar datos generales de HTTP
			responseError.mensajes.push({ codigo: 'HTTP Status ', mensaje: status });
			responseError.mensajes.push({ codigo: 'HTTP StatusText ', mensaje: statusText });
		...
		this.esInformeEditable = function esInformeEditable(fechaActualizacion) {
			var ahora = function ahora() { return new Date(); };
			return ((ahora() - new Date(fechaActualizacion)) < (86400 * $rootScope.config.diasInformeEstudianteEditable * 1000));
		};
	}
]);
