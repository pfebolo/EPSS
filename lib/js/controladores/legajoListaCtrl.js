...
legajosMod.controller('legajoListaControlador', ['$scope', '$http', '$filter', '$location', '$timeout', 'UtilService', 'legajoSrv',
	function CargarLegajos($scope, $http, $filter, $location, $timeout, UtilService, legajoSrv) {
		...
		$scope.cicloDeVida=[];
		var filtro={general: $scope.nombreapellidoFiltro, seguimiento: $scope.filtrarSeguimiento};
		...
		$scope.getCursandoCurso = function getCursandoCurso(cdv) {
			var curso = '';
			cdv.forEach(function(item){ 
				if (item.division.estadoDivisionId==='Cursando')
					curso += UtilService.ordinales[item.anioLectivo-1] + ' Año - ' + UtilService.ordinales[item.nmestreLectivo-1] + ' Cuat. ';
			});
			return curso;
		};
		...
		//param f = Objeto del tipo {general: [stringFiltro], seguimiento:[boolSeguimiento]}
		var seleccionarPP = function seleccionarPP(f) {
			...
			if (f.seguimiento) 
				$scope.legajosFiltrados=$filter('filter')(legajosFiltradosGeneral, {seguimiento: true, }); //filtrar por seguimiento
			else
				$scope.legajosFiltrados= legajosFiltradosGeneral;
		...
		$scope.reseleccionar = function reseleccionar(filtroGeneral,filtroSeguimiento) {
			...
			//Al usar $timeout (con el 3er paramettro en true), ejecutamos la función principal 
			//luego de actualizar la 'vista' (en este ejemplo se actualiza el cursor)
			$timeout(seleccionarPP,0,true,{general: filtroGeneral, seguimiento:filtroSeguimiento})
				.then(function() {
		...
		$scope.irABajarInteresado = function irABajarInteresado() {
			if (!$scope.procesandoCSV) {
				...
				var obj = BOM + 'Legajo' + sep + 'Nombre' + sep + 'Apellido' + sep + 'E-Mail' + sep + 'Teléfono' + sep + 'Celular' + sep + 'DNI' + sep + 'Localidad' + sep + 'Edad' + sep + 'Sexo' + sep + 'Ocupación' + sep + 'Tít. Secundario' + sep + 'Cuestionario' + sep + 'DNI' + sep + 'Título' + sep + 'Foto' + sep + 'Apto Físico' + sep + 'Compromiso' + sep + 'Medio De Contacto' + ret;
