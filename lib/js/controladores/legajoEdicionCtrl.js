...
legajosMod.controller('legajoEdicionControlador', ['$scope', '$http', 'UtilService', 'legajoSrv', 'estudianteSrv',
	function CargarLegajos($scope, $http, UtilSrv, legajoSrv, estudianteSrv) {
		$scope.GetOK = true;
		$scope.cargando = true;
		...
		$scope.cargarLegajo = function cargarLegajo(legajo) {
			...
			$scope.CuestionarioActivo = 0;
			$scope.nacionalidades.forEach(item => { if (item.paisId === legajo.alumno.nacionalidad.paisId) $scope.nacionalidad = item; });
		...
		$scope.ActualizarLegajo = function ActualizarLegajo(legajoAActualizar) {
			...
			delete legajoAActualizar.alumno.nacionalidad;
			legajoAActualizar.estadoEstudianteId = $scope.estadoEstudianteNuevo.estadoEstudianteId;
			legajoAActualizar.alumno.nacionalidadId = $scope.nacionalidad.paisId;
			legajoSrv.update(legajoAActualizar)
				.then(function () {
					...
					legajoAActualizar.alumno.nacionalidad = $scope.nacionalidad;
					window.history.back();
				})
