...
legajosMod.controller('legajoListaControlador', ['$scope', '$http', '$filter', '$location', '$timeout', '$q',  'UtilService', 'legajoSrv', 'grupoSrv', 
	function CargarLegajos($scope, $http, $filter, $location, $timeout, $q,  UtilService, legajoSrv, grupoSrv) {
		...
		var filtro={general: $scope.nombreapellidoFiltro, seguimiento: $scope.filtrarSeguimiento, soloIngresantes: $scope.soloIngresantes};
		...
		$scope.cargarLegajo = function cargarLegajo(legajo) {
			...
			$scope.CuestionarioActivo = 0;
			$http.get($scope.config.servidor + 'api/grupos/byAlumno/' + legajo.alumnoId)
				.then(function (response) {
					legajoSrv.testsrv += 50; //TODO: Eliminar
					$scope.cicloDeVida = $filter('orderBy')(response.data, orderCdeV);
				})
				.catch(function () {
					$scope.cicloDeVida = [];
				});
			ObtenerIntereaccionesXEstudiante(legajo.alumnoId)
				.then(function(response) {
		...
		var filtroIngresante = function filtroIngresante(item) {
			return (item.legajoNro >= legajoSrv.ingresanteMin);
		};

		//param f = Objeto del tipo {general: [stringFiltro], seguimiento:[boolSeguimiento], soloIngresantes:[boolSoloIngresantes]}
		var seleccionarPP = function seleccionarPP(f) {
			var legajosFiltroSeguimiento = [];
			...
			if (typeof leg != 'undefined' && leg.length > 0) {
				...
				legajosFiltradosGeneral = [];
			//Aplico filtro Seguimiento
			if (f.seguimiento) 
				legajosFiltroSeguimiento =$filter('filter')(legajosFiltradosGeneral, {seguimiento: true, }); //filtrar por seguimiento
			else
			legajosFiltroSeguimiento = legajosFiltradosGeneral;
			//Aplico filtro Ingresantes
			if (f.soloIngresantes) 
				$scope.legajosFiltrados=$filter('filter')(legajosFiltroSeguimiento, filtroIngresante); //filtrar por soloIngresantes
			else
				$scope.legajosFiltrados= legajosFiltroSeguimiento;
		...
		$scope.reseleccionar = function reseleccionar(filtroGeneral,filtroSeguimiento, soloIngresantes) {
			...
			$timeout(seleccionarPP,0,true,{general: filtroGeneral, seguimiento:filtroSeguimiento, soloIngresantes:soloIngresantes})
				.then(function() {
		...
		var buscarDivision = function buscarDivision(legajoId,sep) {
			var grupoFiltrado = $filter('filter')(grupos, {alumnoId: legajoId, division : { estadoDivisionId : 'Cursando'}});
		...
		$scope.irABajarInteresado = function irABajarInteresado() {
			if (!$scope.procesandoCSV) {
				...
				var obj = BOM + 'Legajo' + sep + 'Nombre' + sep + 'Apellido' + sep + 'E-Mail' + sep + 'Teléfono' + sep + 'Celular' + sep + 'DNI' + sep + 'Localidad' + sep + 'Edad' + sep + 'Sexo' + sep + 'Ocupación' + sep + 'Tít. Secundario' + sep + 'Cuestionario' + sep + 'DNI' + sep + 'Título' + sep + 'Foto' + sep + 'Apto Físico' + sep + 'Compromiso' + sep + 'Medio De Contacto' + sep + 'Estado' + sep + 'Motivo del Estado' + sep + 'Curso' + sep + 'Modo' + sep + 'División'+ sep + 'Turno';
				obj += ret;
