...
EPSSApp.service('UtilService', function ($q, $http, $rootScope) {
	...
	this.getResponseError = function getResponseError(response,url) {
		...
		if (responseError.hasOwnProperty('mensajes'))
	...
	this.getApi = function getApi(api) {
		...
		$http({
			...
			errorTratado = _this.getResponseError(response,url); 
