...
interesadoMod.controller('interesadoListaControlador', ['$rootScope', '$scope', '$http', '$filter', '$timeout', 'interesadoSrv', 'estudianteSrv', 'interesadoEventoSrv', 'eventoSrv',
	function cargarInteresados($rootScope, $scope, $http, $filter, $timeout, interesadoSrv, estudianteSrv, interesadoEventoSrv, eventoSrv) {
		$scope.GetOK = true;
		...
		var orderApellidoYNombre = function orderApellidoYNombre(item) {
			...
			return item.apellidoYNombre = item.apellido.trim() + ', ' + item.nombre.trim();
		};

		var esDuplicado = function esDuplicado(item, indice, interesados) {
			item.nombreYApellido = item.apellido.trim() + ', ' + item.nombre.trim(); //Permite busqueda con Nombre y Apellido segín capa de presentación
			item.duplicado = (interesados.length > (indice + 1)
				&& item.apellido.trim() === interesados[indice + 1].apellido.trim()
				&& item.nombre.trim() === interesados[indice + 1].nombre.trim()
				&& item.mail.trim() === interesados[indice + 1].mail.trim()
			);
			item.duplicado = (item.duplicado || (indice > 0
				&& item.apellido.trim() === interesados[indice - 1].apellido.trim()
				&& item.nombre.trim() === interesados[indice - 1].nombre.trim()
				&& item.mail.trim() === interesados[indice - 1].mail.trim()
			));
		...
		$scope.reseleccionar = function reseleccionar(filtro) {
			...
			if (filtro !== '' && inte.length > 0) {
				...
				inte = $filter('filter')(inte, stringToLocale(filtro), customComparator);
			...
			fechaIni = performance.now();
			inte = $filter('orderBy')(inte, orderApellidoYNombre);  //ordena Por Apellido+Nombre para buscar duplicados
		...
		$scope.irABajarInteresado = function irABajarInteresado() {
			if (!$scope.procesandoCSV) {
				...
				$scope.interesadosFiltradosYOrdenados.forEach(function (item) {
					obj = obj + item.interesadoId.toString() + ',';
					obj = obj + item.nombre + ',';
					obj = obj + item.apellido + ',';
					obj = obj + item.mail + ',';
					obj = obj + item.telefono + ',';
					obj = obj + item.celular + ',';
					obj = obj + item.comoConocio + ',';
					if (item.modalidad !== null)
						obj = obj + item.modalidad.nombre + ',';
					else
						obj = obj + ',';
					obj = obj + item.gradoInteres + ',';
					obj = obj + item.fechaInteresado + ',';
					obj = obj + item.comentario + ',';
					obj = obj + item.provincia + ',';
					obj = obj + item.situacionInscripcion + ',';
					obj = obj + item.situacionEspecial + ',';
					obj = obj + item.carreraId + ',';
					obj = obj + item.anioAcursar + ',';
					obj = obj + item.nmestreAcursar + ',';
					obj = obj + item.turno + ',';
					obj = obj + item.seguimiento + ',';
					if (item.medioDeContacto !== null)
						obj = obj + item.medioDeContacto.nombre + ',';
					else
						obj = obj + ',';
					if (item.carrera !== null)
						obj = obj + item.carrera.descripcion;
					obj = obj + '\n';
				});
		...
		var obtenerEventosPorInteresado = function obtenerEventosPorInteresado(interesadoId) {
			...
			interesadoEventoSrv.getByInteresadoId(interesadoId)
				.then(function (dataResponseOK) {
					var x = decodeURIComponent(JSON.stringify(dataResponseOK).replace(/\+/g, ' ').replace(/%([^A-F0-9]|[A-F0-9][^A-F0-9])/gi, function myReplace(cad) { return cad.replace('%', '％'); }));
					var intEvt = JSON.parse(x.replace(/\t/g, ' '));
					if (intEvt.length > 0) {
