...
divisionMod.controller('preAsignacionDivisionControlador',['$rootScope', '$scope', '$http', '$q', 'UtilService', 'carreraSrv', 'coordinacionSrv', 'grupoSrv', 'divisionSrv',
	function CargarPreAsignacionDivision($rootScope, $scope, $http, $q, UtilService, carreraSrv, coordinacionSrv, grupoSrv, divisionSrv) {
		...
		var ObtenerModos = function ObtenerModos() {
			...
			divisionSrv.getModos()
				.then(function (dataResponseOK) {
					dataResponseOK.push(todosModos);
				...
				.then(function (dataResponseOK) {
					...
					ObtenerModos()
						.then(function (dataResponseOK) {
							...
							$scope.modoSeleccionado = $scope.modos[$scope.modos.length-1];
							ObtenerEstadosDivision()
								.then(function (dataResponseOK) {
									$scope.estadosDivision = dataResponseOK;
									$scope.estadoDivisionSeleccionado = $scope.estadosDivision[0];
									ObtenerDivisiones()
										.then(function (dataResponseOK) {
		...
		$scope.filtroDivisiones = function (carrera,modo,estado) {
			return function (item) {
				return (item != undefined && carrera != undefined && carrera.carreraId === item.carreraId && estado.estadoDivisionId === item.estadoDivisionId && (modo.modoId===todosModos.modoId || modo.modoId === item.modoId));
			};
		};
