...
informeMod.controller('informeEdicionControlador', ['$scope', '$location', '$filter', '$q', '$timeout', 'legajoSrv', 'coordinadorSrv', 'informeSrv', 'UtilService',
	function cargarInformes($scope, $location, $filter, $q,$timeout, legajoSrv, coordinadorSrv, informeSrv,UtilService) {
		$scope.myStyle = { cursor: 'wait' };
		$scope.error = { hayError: false, errores: {} };
		$scope.cargando = true;
		$scope.esEdicion = function esEdicion() { return informeSrv.accion === informeSrv.acciones.editar; };
		$scope.diasEditable = informeSrv.diasEditable;
		$scope.esEditable = function esEditable() { return UtilService.esInformeEditable($scope.informeEditando.fechaActualizacion);}; 
		var legajoNulo = {
			'alumno': {
				'alumnoId' : null,
				'nombre': '',
