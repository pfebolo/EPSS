...
legajosMod.controller('legajoListaControlador', ['$scope', '$filter', '$location', '$timeout', '$q', 'UtilService', 'legajoSrv', 'grupoSrv', 
	function CargarLegajos($scope, $filter, $location, $timeout, $q, UtilService, legajoSrv, grupoSrv) {
		...
		var ObtenerInformesXEstudiante = function ObtenerInformesXEstudiante(EstudianteId) {
			var deferred = $q.defer();
			legajoSrv.getInformesXEstudiante(EstudianteId)
				.then(function (dataResponseOK) {
					deferred.resolve(dataResponseOK);
				})
				.catch(function (responseError) {
		...
		$scope.obtenerLegajos = function obtenerLegajos() {
			$scope.myStyle.cursor = 'wait';
			$scope.cargando = true;
			legajoSrv.getAll()
		...
		$scope.esInformeEditable = function esInformeEditable(fechaActualizacion) { return UtilService.esInformeEditable(fechaActualizacion);};  


		var cargarInformes = function cargarInformes(estudianteId) {
			ObtenerInformesXEstudiante(estudianteId)
				.then(function (response) {
					$scope.informes = response;
				})
		...
		$scope.cargarLegajo = function cargarLegajo(legajo) {
			...
			cargarInformes(legajo.alumnoId);
		};

		$scope.obtenerEdad = function obtenerEdad(fechaNacimientoString) {
			if ((typeof fechaNacimientoString != 'undefined') && (fechaNacimientoString!==null)) {
		...
		$scope.irABajarInteresado = function irABajarInteresado() {
			if (!$scope.procesandoCSV) {
				...
				var obj = BOM + 'Legajo' + sep + 'Nombre' + sep + 'Apellido' + sep + 'E-Mail' + sep + 'Teléfono' + sep + 'Celular' + sep + 'DNI' + sep + 'Localidad' + sep + 'Cod. Postal' + sep + 'Fecha Nacimiento' + sep + 'Edad' + sep + 'Sexo' + sep + 'Ocupación' + sep + 'Tít. Secundario' + sep + 'Cuestionario' + sep + 'DNI' + sep + 'Título' + sep + 'Foto' + sep + 'Apto Físico' + sep + 'Compromiso' + sep + 'Fecha Ingreso' + sep + 'Medio De Contacto' + sep + 'Modo (inscripción)' + sep + 'Estado' + sep + 'Motivo del Estado' + sep + 'Curso' + sep + 'Modo' + sep + 'División' + sep + 'Turno' + sep + 'Nacionalidad' + sep + 'Libro' + sep + 'Folio' ;
				obj += ret;
				$scope.legajosFiltrados.forEach(function (item) {
					...
					obj = obj + UtilService.NullToBlanck($filter('date')(item.fechaNacimiento, 'dd-MM-yyyy')) + sep;
					obj = obj + UtilService.NullToBlanck($scope.obtenerEdad(item.fechaNacimiento)) + sep;
					...
					obj = obj + UtilService.NullToBlanck($filter('date')(item.fechaIngreso, 'dd-MM-yyyy')) + sep;
					...
					if (item.alumno.modalidad!==null)
						obj = obj + UtilService.NullToBlanck(item.alumno.modalidad.nombre) + sep;
					else
						obj = obj + sep;
		...
		var eliminarLegajoPINSecreto = legajoSrv.pinEliminacion;
		var legajoAEliminar = null;
		$scope.eliminarLegajoPIN = null;
		$scope.eliminarLegajoPINErroneo = false;
		$scope.modalEliminarLegajoOpen = false;
		$scope.showModalEliminarLegajo = function showModalEliminarLegajo(legajoSeleccionado) {
			legajoAEliminar = legajoSeleccionado;
			$scope.modalEliminarLegajoOpen = true;
		};

		

		var eliminarLegajo = function eliminarLegajo(legajoAEliminar) {
			$scope.myStyle.cursor = 'wait';
			$scope.cargando = true;
			legajoSrv.eliminar(legajoAEliminar.alumno)
				.then(function () {
					//$scope.obtenerLegajos();
					var indiceDeElementoABorrar = $scope.legajos.findIndex(legajo => {return legajo.alumnoId==legajoAEliminar.alumnoId;});
					$scope.legajos.splice(indiceDeElementoABorrar,1);
					indiceDeElementoABorrar = legajosFiltradosGeneral.findIndex(legajo => {return legajo.alumnoId==legajoAEliminar.alumnoId;});
					legajosFiltradosGeneral.splice(indiceDeElementoABorrar, 1);
					$scope.reseleccionar($scope.nombreapellidoFiltro,$scope.filtrarSeguimiento,$scope.soloIngresantes,$scope.filtrarCuestionario);
					$scope.selectedRow=0;
				})
				.catch(function (responseError) {
					$scope.error.errores = responseError.mensajes;
		...
		$scope.closeModalEliminarLegajo = function closeModalEliminarLegajo(eliminarLegajoPIN) {
			if (eliminarLegajoPINSecreto === btoa(eliminarLegajoPIN)) {
				eliminarLegajo(legajoAEliminar);
				$scope.modalEliminarLegajoOpen = false;
			} else if (eliminarLegajoPIN === 'Cancelar')
				$scope.modalEliminarLegajoOpen = false;
			else
				$scope.eliminarLegajoPINErroneo = true;
			$scope.eliminarLegajoPIN = null;
		};
		//Fin Modal Eliminar Legajo

		//INI-Informes de un Estudiante.
		//Modal Informes.
		$scope.informeEditando = null;
		$scope.modalInformeOpen = false;
		$scope.soloMostrar=true;
		var informeOriginal = false;

		$scope.showModalInforme = function showModalInformeEditar(informe) {
			$scope.informeEditando = informe;
			$scope.modalInformeOpen = true;
		};

		$scope.showModalInformeEditar = function showModalInformeEditar(informe) {
			//$scope.interesadoEventoEditandoOriginal = interesadoEventoAEditar;
			var fechaId = new Date();
			...
			var informeBase = {
				...
				'anioLectivo': informe.anioLectivo,
				'coordinadorId': informe.coordinadorId,
				'informe': informe.informe,
				'fechaCreacion': informe.fechaCreacion,
				'fechaActualizacion': fechaId.toJSON(),
				'coordinador': informe.coordinador
				//'fecha': fechaId.toJSON(), //$filter('date')(fechaId, 'yyyy-MM-ddTHH:mm:ss'),
			};
			$scope.informeEditando = informeBase;
			$scope.soloMostrar=false;
			$scope.modalInformeOpen = true;
		};

		$scope.closeModalInforme = function closeModalInforme() {
			$scope.modalInformeOpen = false;
			$scope.soloMostrar=true;
		};
		//Fin Modal Interacción.

		$scope.actualizarInforme = function actualizarInforme(informeAActualizar) {
			$scope.myStyle.cursor = 'wait';
			legajoSrv.actualizarInforme(informeAActualizar)
				.then(function () {
					informeOriginal.informe = informeAActualizar.informe;
				})
				.catch(function (responseError) {
					$scope.error.errores = responseError.mensajes;
					$scope.error.hayError = true;
		...
		$scope.irACrearInforme = function irACrearinforme() {
			legajoSrv.exponerCreacionInforme();
		};
