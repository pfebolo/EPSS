...
interesadoMod.controller('interesadoListaControlador', ['$rootScope', '$scope', '$http', '$filter', '$timeout', '$q', 'UtilService', 'interesadoSrv', 'estudianteSrv', 'interesadoEventoSrv', 'eventoSrv',
	function cargarInteresados($rootScope, $scope, $http, $filter, $timeout, $q, UtilService, interesadoSrv, estudianteSrv, interesadoEventoSrv, eventoSrv) {
		$scope.GetOK = true;
		...
		//TOD: Eliminar al pasar todo al controla del componente de interacciones
		$scope.cargarInteraccionesWrapper = function cargarInteraccionesWrapper(interesadoId) {
			cargarInteracciones(interesadoId);
		};

		var cargarInteracciones = function cargarInteracciones(interesadoId) {
		...
		$scope.cargarInteresado = function cargarInteresado(interesado) {
			...
			if (interesado !== null) {
				...
				cargarInteracciones(interesado.interesadoId);
			}
			else
		...
		// $scope.showModalInteraccion = function showModalInteraccion(interesado) {
		...
		// 		.then(function () {
		// 			cargarInteracciones(interaccionAEliminar.interesadoId);
		// 		})
		// 		.catch(function (responseError) {
		// 			$scope.error.errores = responseError.mensajes;
		// 			$scope.error.hayError = true;
		...
		//FIN-Interacción de un Interesado.
